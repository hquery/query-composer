$.widget("ui.DemographicsEditor",{
  ...
  _create: function(){
    ...
    this.age_slider = $("<div style='margin:10px'>").slider({
      min:0, 
      max:130, 
    ...
    this.genderDiv = $("<p>").append("<label>Gender</label>");
    this.genderDiv.append(this._createGenderSelect(this.gender));

    this.raceDiv = $("<p>").CodeList({title:"Race",type:"race",selected:this.raceCode, onChange:function(code,event){self.raceCode = code; self._update()}});
    this.msDiv = $("<p>").CodeList({title:"Marital Staus",type:"marital_status",selected:this.maritalStatusCode, onChange:function(code,event){self.maritalStatusCode = code; self._update()}});
