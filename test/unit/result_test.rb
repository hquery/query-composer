...
class ResultTest < ActiveSupport::TestCase
  ...
  test "check a result that has changed" do
    FakeWeb.register_uri(:get, HTTP_PROTO_CLIENT+"://localhost:3000/queries/4e4c08b5431a5f5dc1000001",
                         :body => '{"status": "'+Result::COMPLETE+'", "result_url": "'+HTTP_PROTO_CLIENT+'://localhost/results/1234"}')
    FakeWeb.register_uri(:get, HTTP_PROTO_CLIENT+"://localhost/results/1234",
                         :body => '{"foo": "bar"}')
  ...
  test "fetch a result" do
    FakeWeb.register_uri(:get, HTTP_PROTO_CLIENT+"://localhost/results/1234",
                         :body => '{"foo": "bar"}')
    result = FactoryGirl.create(:result, :result_url => HTTP_PROTO_CLIENT+"://localhost/results/1234")
    result.fetch_result
    assert_equal Result::COMPLETE, result.status
    assert_equal 'bar', result.value['foo']
  end
  ...
  test "checking a result" do
    FakeWeb.register_uri(:get, HTTP_PROTO_CLIENT+"://localhost:3000/queries/4e4c08b5431a5f5dc1000001",
                         :body => '{"status": "complete", "result_url": "'+HTTP_PROTO_CLIENT+'://localhost/results/1234"}')
    FakeWeb.register_uri(:get, HTTP_PROTO_CLIENT+"://localhost/results/1234",
